...
class editSubscribe extends React.Component {
  ...
  constructor(props) {
    ...
    let disable =  this.props.navigation.getParam('isdisable')
    this.state = {
    ...
    this.props.fetchSubscriptionDtlsById(this.state.itemId).then(res => {
      ...
      if (res.status == "success") {
        ...
        this.setState({
          ...
          isActive: 1,
          //isdisable:moment(subScriDet.startDate).format('DD/MM/YYYY') < moment(new Date()).format('DD/MM/YYYY'),
          qty: subScriDet.quantity,
          displaystartDate: moment(subScriDet.startDate).format('DD MMM YYYY'),
        });
        console.warn(this.state.isdisable)
      }
    });
  })
