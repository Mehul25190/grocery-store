...
class SubscribeOrder extends React.Component {
  ...
  constructor(props) {
    ...
    let qty = this.props.navigation.getParam('qty')
     ...
     this.state = {
      ...
      endDate: moment(new Date(), "DD/MM/YYYY").add(16, 'days').format('DD/MM/YYYY'), 
  ...
  setStartDate(value){
    ...
    console.log(value);
    this.setState({startDate: val1, displaystartDate: moment(value).format('DD MMM YYYY'), endDate: moment(value).add(this.state.duration, 'days').format('DD/MM/YYYY')})
  }
  ...
  subscribeSubmitHandler(){
    ...
    //var end_date = moment(this.state.endDate).format('MM/DD/YYYY');
